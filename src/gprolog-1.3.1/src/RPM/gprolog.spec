...
Source: ftp://gprolog.univ-paris1.fr/pub/gprolog/gprolog-1.3.1.tar.gz
...
%build
cd src
./configure --with-install-dir=$RPM_BUILD_ROOT//usr/lib/gprolog-1.3.1 \
            --without-links-dir --without-examples-dir \
            --with-doc-dir=$RPM_BUILD_ROOT//usr/doc/gprolog-1.3.1
make
...
cd $RPM_BUILD_ROOT//usr/lib/gprolog-1.3.1/bin
for i in *; do
	ln -s ../lib/gprolog-1.3.1/bin/$i $RPM_BUILD_ROOT//usr/bin/$i
done
cd $RPM_BUILD_DIR/gprolog-1.3.1
cp README COPYING ChangeLog NEWS $RPM_BUILD_ROOT//usr/doc/gprolog-1.3.1
...
%files
%defattr(-,root,root)
%doc /usr/doc/gprolog-1.3.1/*
/usr/bin/*
/usr/lib/gprolog-1.3.1*
